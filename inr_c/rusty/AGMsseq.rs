...
#[no_mangle]
pub unsafe extern "C" fn GMcmp(mut u: *mut SHORT, mut v: *mut SHORT) -> libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn A_GMsseq(mut A: A_OBJECT) -> A_OBJECT {
    ...
    let mut queue_lim: libc::c_int = 0;
    ...
    queue_lim = 5 as libc::c_int * (*A).A_nQ;
                ...
                if !((*p).A_a == (*p).A_c) {
                    if *set.offset((*p).A_c as isize) == 0o17777777777 as libc::c_int {
                        ...
                        *queue.offset(fresh1 as isize) = (*p).A_c;
                        if nq >= queue_lim {
                            Error(
                                b"queue_lim exceeded (1)\0" as *const u8
                    ...
                    *fresh2 = s_alloc(len + 1 as libc::c_int);
                    ...
                    if !save_coeff.is_null() {
                        if GMcmp(save_coeff, *to_coeff.offset((*p).A_c as isize))
                            >= 0 as libc::c_int
                        {
                            Sfree(save_coeff as *mut libc::c_char);
                            let fresh3 = nq;
                            nq = nq + 1;
                            *queue.offset(fresh3 as isize) = (*p).A_c;
                            if nq >= queue_lim {
                                Error(
                                    b"queue_lim exceeded (2)\0" as *const u8
                                        as *const libc::c_char as *mut libc::c_char,
                                );
                            }
                        } else {
                            ...
                            let ref mut fresh4 = *to_coeff.offset((*p).A_c as isize);
                            *fresh4 = save_coeff;
                        }
                    }
                }
    ...
    *vec
    ...
    let ref mut fresh7 = *sig.offset(2 as libc::c_int as isize);
    *fresh7 = 0o17777777777 as libc::c_int;
    let ref mut fresh8 = *sig.offset(1 as libc::c_int as isize);
    *fresh8 = *fresh7;
    *sig.offset(0 as libc::c_int as isize) = *fresh8;
    let ref mut fresh9 = *back.offset(2 as libc::c_int as isize);
    *fresh9 = 0 as libc::c_int;
    let ref mut fresh10 = *back.offset(1 as libc::c_int as isize);
    *fresh10 = *fresh9;
    *back.offset(0 as libc::c_int as isize) = *fresh10;
    while current < (*V).V_n {
        if !(current == 1 as libc::c_int) {
            ...
            while i < len {
                ...
                {
                    hsize += 1;
                    let ref mut fresh11 = *heap.offset(hsize as isize);
                    *fresh11 = *((*A).A_p).offset(j as isize);
                }
                let ref mut fresh12 = *fr_coeff.offset(j as isize);
                *fresh12 = V_vec(Vs, *fvec.offset((len + i) as isize));
                i += 1;
            }
            if !(hsize == 0 as libc::c_int) {
                ...
                while base > 0 as libc::c_int {
                    ...
                    loop {
                        ...
                        *fresh13 = *heap.offset(son as isize);
                        father = son;
                    }
                    let ref mut fresh14 = *heap.offset(father as isize);
                    *fresh14 = insert;
                    base -= 1;
                }
                ...
                loop {
                        ...
                        if 8 as libc::c_int * vlen < (*A).A_nQ {
                            ...
                            while head < 0o17777777777 as libc::c_int - 1 as libc::c_int
                            {
                                ...
                                if head == 1 as libc::c_int || p < pz {
                                    let fresh15 = n;
                                    n = n + 1;
                                    *vec.offset(fresh15 as isize) = head;
                                } else {
                        ...
                        || bb % 2 as libc::c_int == 0 as libc::c_int
                    {
                        ...
                        *fresh17 = s_alloc(len + 1 as libc::c_int);
                        veccpy(
                        ...
                        GMval = 1 as libc::c_int;
                        if !save_coeff.is_null() {
                            GMval = GMcmp(save_coeff, *to_coeff.offset(cc as isize));
                            if GMval >= 0 as libc::c_int {
                                Sfree(save_coeff as *mut libc::c_char);
                            } else {
                                Sfree(*to_coeff.offset(cc as isize) as *mut libc::c_char);
                                let ref mut fresh18 = *to_coeff.offset(cc as isize);
                                *fresh18 = save_coeff;
                            }
                        }
                        if GMval != 0 as libc::c_int {
                                        ...
                                        if !((*p).A_a == (*p).A_c) {
                                                ...
                                                == 0o17777777777 as libc::c_int
                                            {
                                                *set.offset((*p).A_c as isize) = head;
                                                ...
                                                let fresh19 = nq;
                                                nq = nq + 1;
                                                *queue.offset(fresh19 as isize) = (*p).A_c;
                                                if nq >= queue_lim {
                                                    Error(
                                                        b"queue_lim exceeded (3)\0" as *const u8
                                                            as *const libc::c_char as *mut libc::c_char,
                                                    );
                                                }
                                            } else {
                                            ...
                                            *fresh20 = s_alloc(len + 1 as libc::c_int);
                                            veccpy(
                                                *to_coeff.offset((*p).A_c as isize),
                                                *to_coeff.offset((*p).A_a as isize),
                                            );
                                            ...
                                            if !save_coeff.is_null() {
                                                    ...
                                                    >= 0 as libc::c_int
                                                {
                                                    ...
                                                    let fresh21 = nq;
                                                    nq = nq + 1;
                                                    *queue.offset(fresh21 as isize) = (*p).A_c;
                                                    if nq >= queue_lim {
                                                        Error(
                                                            b"queue_lim exceeded (4)\0" as *const u8
                                                                as *const libc::c_char as *mut libc::c_char,
                                                        );
                                                    }
                                                } else {
                                                    ...
                                                    let ref mut fresh22 = *to_coeff.offset((*p).A_c as isize);
                                                    *fresh22 = save_coeff;
                                                }
                                            }
                                        }
                            ...
                            )
                    {
                        ...
                        let fresh23 = hsize;
                        hsize = hsize - 1;
                        insert = *heap.offset(fresh23 as isize);
                    }
                    last = *heap.offset(1 as libc::c_int as isize);
                    ...
                    loop {
                        ...
                        let ref mut fresh24 = *heap.offset(father as isize);
                        *fresh24 = *heap.offset(son as isize);
                        father = son;
                    }
                    let ref mut fresh25 = *heap.offset(father as isize);
                    *fresh25 = insert;
                }
            }
        }
