...
#[no_mangle]
pub unsafe extern "C" fn A_sseq(mut A: A_OBJECT) -> A_OBJECT {
    ...
    while i < nq {
        ...
        while p < pz {
                ...
                && (*p).A_b % 2 as libc::c_int == 1 as libc::c_int
            {
                if (*p).A_a == (*p).A_c {
                    Error(
                        b"A_sseq: Relation not single-valued (S2)\0" as *const u8
    ...
    *vec
    ...
    let ref mut fresh5 = *sig.offset(2 as libc::c_int as isize);
    *fresh5 = 0o17777777777 as libc::c_int;
    let ref mut fresh6 = *sig.offset(1 as libc::c_int as isize);
    *fresh6 = *fresh5;
    *sig.offset(0 as libc::c_int as isize) = *fresh6;
    let ref mut fresh7 = *back.offset(2 as libc::c_int as isize);
    *fresh7 = 0 as libc::c_int;
    let ref mut fresh8 = *back.offset(1 as libc::c_int as isize);
    *fresh8 = *fresh7;
    *back.offset(0 as libc::c_int as isize) = *fresh8;
    while current < (*V).V_n {
        if !(current == 1 as libc::c_int) {
            ...
            while i < len {
                ...
                {
                    hsize += 1;
                    let ref mut fresh9 = *heap.offset(hsize as isize);
                    *fresh9 = *((*A).A_p).offset(j as isize);
                }
                let ref mut fresh10 = *fr_coeff.offset(j as isize);
                *fresh10 = V_vec(Vs, *fvec.offset((len + i) as isize));
                i += 1;
            }
            if !(hsize == 0 as libc::c_int) {
                ...
                while base > 0 as libc::c_int {
                    ...
                    loop {
                        ...
                        let ref mut fresh11 = *heap.offset(father as isize);
                        *fresh11 = *heap.offset(son as isize);
                        father = son;
                    }
                ...
                loop {
                        ...
                        if 8 as libc::c_int * vlen < (*A).A_nQ {
                            ...
                            while head < 0o17777777777 as libc::c_int - 1 as libc::c_int
                            {
                                ...
                                if head == 1 as libc::c_int || p < pz {
                                    let fresh13 = n;
                                    n = n + 1;
                                    *vec.offset(fresh13 as isize) = head;
                                } else {
                        ...
                        || bb % 2 as libc::c_int == 0 as libc::c_int
                    {
                        ...
                        let ref mut fresh15 = *to_coeff.offset(cc as isize);
                        *fresh15 = s_alloc(len + 1 as libc::c_int);
                        veccpy(
                            ...
                            while i < nq {
                                ...
                                while p < pz {
                                        ...
                                        && (*p).A_b % 2 as libc::c_int == 1 as libc::c_int
                                    {
                                        if (*p).A_a == (*p).A_c {
                                            Error(
                                                b"A_sseq: Relation not single-valued (2)\0" as *const u8
                                                    as *const libc::c_char as *mut libc::c_char,
                                            );
                                        }
                                            ...
                                            == 0o17777777777 as libc::c_int
                                        {
                                            *set.offset((*p).A_c as isize) = head;
                                            ...
                                            let fresh16 = nq;
                                            nq = nq + 1;
                                            *queue.offset(fresh16 as isize) = (*p).A_c;
                                        } else {
                                            save_coeff = *to_coeff.offset((*p).A_c as isize);
                                        }
                                        len = veclen(*to_coeff.offset((*p).A_a as isize))
                                            ...
                                            + *st_len.offset((*p).A_c as isize);
                                        let ref mut fresh17 = *to_coeff.offset((*p).A_c as isize);
                            ...
                            )
                    {
                        ...
                        let fresh18 = hsize;
                        hsize = hsize - 1;
                        insert = *heap.offset(fresh18 as isize);
                    }
                    last = *heap.offset(1 as libc::c_int as isize);
                    ...
                    loop {
                        ...
                        let ref mut fresh19 = *heap.offset(father as isize);
                        *fresh19 = *heap.offset(son as isize);
                        father = son;
                    }
                    let ref mut fresh20 = *heap.offset(father as isize);
                    *fresh20 = insert;
                }
            }
        }
